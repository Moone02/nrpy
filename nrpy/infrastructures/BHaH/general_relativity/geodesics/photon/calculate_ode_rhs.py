...
def calculate_ode_rhs(
    ...
    # Identify which symbols are actually used to avoid unnecessary C code
    ...
    # 3. Unpack Christoffel Symbols (conn_Gamma4UDD)
    ...
    # Define where the output k-vectors go
    k_array_outputs = [
        f"k_array[IDX_LOCAL((stage - 1) * 9 + {i}, j, bundle_capacity)]" for i in range(9)
    ]

    # Generate the optimized C kernel
    body += ccg.c_codegen(
